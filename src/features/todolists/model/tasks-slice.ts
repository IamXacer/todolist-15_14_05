...
export const tasksSlice = createAppSlice({
  ...
  reducers: (create) => ({
    fetchTasksTC: create.asyncThunk(
      async (todolistId: string, { dispatch, rejectWithValue }) => {
        try {
          ...
          console.error("Error in fetchTasksTC:", error); // Логування помилки
    ...
    createTaskTC: create.asyncThunk(
      ...
      {
        fulfilled: (state, action) => {
          state[action.payload.task.todoListId].unshift(action.payload.task) // Например, добавление новой задачи в состояние
        },
      }
    ),
    ...
    deleteTaskTC: create.asyncThunk(
      async (payload: { todolistId: string; taskId: string }, { dispatch, rejectWithValue }) => {
        try {
          ...
          console.error("Error in deleteTaskTC:", error); // Логування помилки
          dispatch(setAppStatusAC({ status: "failed" }))
          return rejectWithValue(null)
        }
      },
